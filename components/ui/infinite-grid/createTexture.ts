...
/**
 * Only set `crossOrigin = "anonymous"` for cross-origin URLs. Using it for
 * same-origin paths (e.g. `/sponsored/...`) can still trigger CORS checks on
 * some stacks and breaks canvas/WebGL when headers are missing — common when
 * `siteUrl` points at production while developing on localhost.
 */
function applyImageSrc(img: HTMLImageElement, src: string): void {
  const resolvedSrc = src || "/photo.png";
  if (typeof window === "undefined") {
    img.src = resolvedSrc;
    return;
  }
  try {
    const resolved = new URL(resolvedSrc, window.location.href);
    if (resolved.origin !== window.location.origin) {
      img.crossOrigin = "anonymous";
    }
  } catch {
    // invalid URL — load without crossOrigin
  }
  img.src = resolvedSrc;
}
...
function wrapCardDescriptionLines(
 ...
 * - Wrapped description text at the bottom (tags are not drawn)
 ...
 *
 * @param data - Card data containing title, image, description, date, etc.
...
export async function generateForegroundTexture(
  ...
  const cacheKey = `${data.title}-${data.description ?? ""}-${data.date}`;
  ...
  applyImageSrc(imageObj, data.image || "/photo.png");
  ...
  const descLineHeight = 17;
  const descMaxLines = 4;
  const descMaxWidth = cardWidth - padding * 2 - 72;

  if (data.description?.trim()) {
    ctx.font = `${descFontSize}px Helvetica, Arial, sans-serif`;
    ctx.textAlign = "left";
    ctx.textBaseline = "top";
    const lines = wrapCardDescriptionLines(ctx, data.description, descMaxWidth, descMaxLines);
    const blockH = lines.length * descLineHeight;
    const dateRowH = 26;
    let y = cardHeight - padding - dateRowH - 6 - blockH;
...
export async function generateBackgroundTexture(
  ...
  applyImageSrc(backgroundImageObj, data.image || "/photo.png");
